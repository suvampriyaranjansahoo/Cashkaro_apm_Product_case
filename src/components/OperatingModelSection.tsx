  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-[#0E1726] border border-[#DEB6C5]/70 dark:border-slate-800 rounded-2xl p-6 shadow-sm mb-8 transition-colors">
          <div className="flex items-center justify-between border-b border-[#DEB6C5]/40 dark:border-slate-800 pb-4 mb-6">
            <div>
              <span className="text-[10px] font-mono uppercase tracking-wider text-[#8F3760] dark:text-blue-400 font-semibold">Phased Execution Pipeline</span>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Discover → De-Risk → Build/Pilot → Experiment → Scale</h3>
            </div>
            <span className="text-xs font-mono text-slate-600 dark:text-slate-400">Stage Gate Governance</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            {phases.map((p) => (
              <div key={p.phase} className="p-4 rounded-xl bg-[#F7F6ED] dark:bg-slate-900/60 border border-[#DEB6C5]/60 dark:border-slate-800 flex flex-col justify-between space-y-3">
                <div>
                  <div className="flex items-center justify-between text-[11px] font-mono text-slate-500 dark:text-slate-400 mb-1">
                    ...
                    <span className="w-2 h-2 rounded-full bg-[#D190AC] dark:bg-[#0080AB]"></span>
                  </div>
                  <h4 className="font-display font-bold text-sm text-slate-900 dark:text-white">{p.phase}</h4>
                  
                  <div className="mt-2 text-[11px] space-y-1">
                    <span className="text-[10px] font-mono uppercase text-slate-500 dark:text-slate-400 font-semibold block">Owner:</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200 block">{p.owner}</span>
                  </div>

                  <div className="mt-2 text-[11px] space-y-1">
                    <span className="text-[10px] font-mono uppercase text-slate-500 dark:text-slate-400 font-semibold block">Deliverable:</span>
                    <p className="text-slate-700 dark:text-slate-400 leading-snug">{p.deliverable}</p>
                  </div>
                </div>

                <div className="pt-2 border-t border-[#DEB6C5]/50 dark:border-slate-800 text-[10px] font-mono font-bold text-emerald-700 dark:text-[#159A68]">
                  Exit: {p.exit}
                </div>
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-[#0E1726] border border-[#DEB6C5]/70 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm mb-8 transition-colors">
          <div className="p-4 sm:p-5 bg-[#F0EAD5] dark:bg-slate-950 text-slate-900 dark:text-white border-b border-[#DEB6C5]/80 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="text-xs text-slate-600 dark:text-slate-400 font-mono">Explicit Cadences</span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-[#F7F6ED] dark:bg-slate-900 border-b border-[#DEB6C5]/60 dark:border-slate-800 font-mono text-[11px] text-slate-700 dark:text-slate-400 uppercase">
                <tr>
              ...
              <tbody className="divide-y divide-[#DEB6C5]/30 dark:divide-slate-800 text-slate-800 dark:text-slate-300">
                {risks.map((r) => (
                  <tr key={r.risk} className="hover:bg-[#F7F6ED]/60 dark:hover:bg-slate-900/50">
                    <td className="p-3.5 sm:px-4 font-bold text-slate-900 dark:text-white">
                      {r.risk}
                    </td>
                    <td className="p-3.5 sm:px-4 text-slate-700 dark:text-slate-400 leading-relaxed max-w-sm">
                      {r.mitigation}
                    </td>
                    <td className="p-3.5 sm:px-4 font-semibold text-slate-900 dark:text-slate-200">
                      {r.owner}
                    </td>
                    <td className="p-3.5 sm:px-4 font-mono text-[11px] text-slate-700 dark:text-slate-400">
                      {r.cadence}
                    </td>
                    <td className="p-3.5 sm:px-4 text-center">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase font-mono ${
                        r.severity === 'Critical' ? 'bg-rose-100 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-900/40' : r.severity === 'High' ? 'bg-amber-100 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border border-amber-200 dark:border-amber-900/40' : 'bg-[#F0D6DE]/60 dark:bg-slate-800 text-slate-800 dark:text-slate-300 border border-[#DEB6C5]/50 dark:border-slate-700'
                      }`}>
                        {r.severity}
                      </span>
                    </td>
        ...
        <div className="p-4 sm:p-5 rounded-xl bg-[#F0EAD5] dark:bg-amber-950/30 border border-[#DEB6C5] dark:border-amber-900/60 text-xs text-slate-900 dark:text-amber-200 transition-colors">
          <div className="flex items-center gap-2 font-bold mb-1 font-mono uppercase tracking-wider text-[#8F3760] dark:text-amber-300">
            <CheckCircle2 className="w-4 h-4 text-emerald-700 dark:text-amber-400" />
